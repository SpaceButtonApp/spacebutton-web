...
import {
  ...
  MapPin, Bed, Bath, CheckCheck, X, RefreshCw, AlertCircle,
...
export default function ListingsView() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      while (all.length < (firstPage.value.total ?? 0) && (firstPage.value.listings?.length ?? 0) >= 100) {
        const next = await supportApi.getListings(page, 100);
  ...
  async function handleReject(id: string, reason: string) {
    try {
      await supportApi.rejectListing(id, reason);
  ...
  async function handleClose(id: string) {
    try {
      await supportApi.closeListing(id);
  ...
  async function handleDelete(id: string) {
    try {
      await supportApi.deleteListing(id);
  ...
  function handleExport() {
    exportToExcel("listings", filtered.map((l) => ({
      ...
      PropertyType: l.categoryDisplay, Status: l.status, Approval: l.approval,
...
function ListingDetailPage({
  listing, onBack, onApprove, onReject, onClose, onDelete,
}: {
  listing: ListingRow;
  onBack: () => void;
  onApprove: () => void;
  ...
  const ownerLabel = data.ownerType === "agent" ? "Agent" : "User";
